...
var how;
var about;
var running , puzzleaud , climber , treasure , beginning;


var htp, ply, backButton, atg, lvl1,clock1 , flower1 ,  htpimg, atgimg, randy2, pl7;
var backButton, nextButton,cp1 ,cpGroup , randy4 , clockimg,flowerimg , cpimg, flowerGroup ,randy3 , clockGroup ,clock , flower , cp ,cpscore , flowerscore , clockscore ,   pl10 , flower1img, flower0img;
var spine,
  spineGroup,
  spine1img,
  spine2img,
...
function preload() {
  ...
  csbimg = loadImage("clickspacebar.gif");
  ...
  spine1img = loadImage("spine1.png");
    spine2img = loadImage("spine2.png");
  ...
  flower0img = loadImage("flower0.png");
  flower1img = loadImage("flower1.png");
  clock0img = loadImage("clock0.png");
  clock1img = loadImage("clock1.png");
  cp0img = loadImage("caterrpillar0.png");
    cp1img = loadImage("caterrpillar1.png");
...
function setup() {
  ...
  beginning = new Audio('beginning.mp3');
  beginning.loop = true;
  beginning.volume = 0.3;

  climber = new Audio('climber.mp3');
  climber.loop = true;
  climber.volume = 0.3;
  ...
  puzzleaud = new Audio('puzzle.mp3');
  puzzleaud.loop = true;
  puzzleaud.volume = 0.3;

  running = new Audio('running.mp3');
  running.loop = true;
  running.volume = 0.4;

  gameOverAud = new Audio('gameOver.mp3');
  gameOverAud.loop = false;
  gameOverAud.volume = 0.4;
  ...
  music = createImg("music.png");
  music.position(width/2- - 870 , height /2 - -350);
  music.size(70 , 100);
  music.class("music");
  music.hide();

    noMusic = createImg("nomusic.png");
  noMusic.position(width/2- - 870 , height /2 - -350);
  noMusic.size(70 , 100);
  noMusic.class("music");
  ...
  portal.hide();
    ...
    pl10 = createVideo("pl10.mp4");
  pl10.position(-50, -100);
  pl10.size(width + 70, height + 200);
    pl10.hide();

      pl11 = createVideo("pl11.mp4");
  pl11.position(-50, -100);
  pl11.size(width + 70, height + 200);
    pl11.hide();

      pl12 = createVideo("pl12.mp4");
  pl12.position(-50, -100);
  pl12.size(width + 70, height + 200);
  pl12.hide();
  ...
  b11 = createSprite(width / 2 + 800, height - 900);
  b11.addImage(b11img);
  b11.visible = false;

  cp = createSprite(width / 2 + 400, height - 900);
  cp.addImage(cp0img);
  cp.scale = 0.5;
 cp.visible = false;

  flower = createSprite(width / 2 + 800, height - 900);
  flower.addImage(flower0img);
  flower.scale = 0.5;
  flower.visible = false;

    clock = createSprite(width / 2 + 600, height - 900);
  clock.addImage(clock0img);
  clock.scale = 0.5;
  clock.visible = false;
  ...
  csb = createSprite(width/2 , height/2 + 100 , 100 , 100)
  csb.addImage(csbimg)
  csb.scale = 2.50;
  csb.visible = false;
...
door1.position(width/2 - 960,height-720)
door1.size(500,600);
door1.class("invisiblebutton");
door1.hide();

door2=createButton("TWO")
door2.position(width/2 - 280,height-800)
door2.size(600,700);
door2.class("invisiblebutton");
door2.hide();


door3=createButton("Three")
door3.position(width-500,height-720)
door3.size(500,600);
door3.class("invisiblebutton");
door3.hide();
}

function level2() {
  wall.visible = true;
        ddr2.collide(invisibleGround2)
            ddr2.collide(invisibleGround3)
                  ddr2.collide(invisibleGround4)
  backButton.hide();
  ...
  if (frameCount % 420 == 0) {
    ...
    switch (rand) {
      case 1:
        ...
        break;
      case 2:
        spine.addImage(spine1img);
        break;
      case 3:
        spine.addImage(spine2img);
        break;
      default:
        break;
    }
...
function validate() {
  ...
  if (cdText !== "KTTSMTB") {
    Swal.fire({
      icon: "error",
      title: "Wrong",
      text: "Tips : Check if you left a space of wrote a extra letter",
    });
  }
...
function draw() {
  ...
  if(gameState !== "wait" && gameState !== "playstate"){
    beginning.pause();
  }

    if(gameState !== "playstate1" && gameState !== "video2"  && gameState !== "none"){
    running.pause();
  }

      if(gameState !== "lvl2wait" && gameState !== "playstate2"){
    climber.pause();
  }
  ...
  nextButton.mousePressed(() => {
      ...
      text: "You need to get 5 coins and you have only ten bubble . You can shoot a bubble by pressing right arrow and click on space bar for jumping",
    }).then((result) => {
      if (result.isConfirmed) {
  ...
  if (gameState === "playstate5") {
    kpf.mousePressed(() => {
      ...
      pl9.play();
          puzzleaud.pause();
    treasure.play();
    gameState = "lastplaystate"
    });
  }

  if(gameState === "lastplaystate"){
    door1.show();
    door2.show();
    door3.show();
    door1.mousePressed(() =>{
  ...
  portal.mousePressed(() => {
    pl4.stop();
        Swal.fire({
      icon: "info",
      title: "Hot to play this game ?",
      text: "You ned to take the hint paper without touching the spines . Click the space bar for jumping and left arrow to move left and right arrow for moving right",
    }).then((result) => {
      if (result.isConfirmed) {
        climber.play(); 
            gameState = "lvl2wait";
...
csb.visible = true;
  ...
  if (gameState === "playstate2") {
    ...
    wall.velocityY = -7;
    if (frameCount % 400 == 0) {
      randy2 = Math.round(random(5, height - height / 10));
       var flower1 = createSprite(randy2, 20, 20, 20);
      flower1.scale = 0.49;
      flower1.addImage(flowerimg)
      flower1.velocityY = 13;

      flower1.add(flowerGroup);
    }

        if (frameCount % 350 == 0) {
      var randy3 = Math.round(random(5, height - height / 7));
      var clock1 = createSprite(randy3, 20, 20, 20);
      clock1.scale = 0.49;
      clock1.addImage(clockimg);
      clock1.velocityY = 13;

      clock1.add(clockGroup);
    }

                if (frameCount % 300 == 0) {
      var randy4 = Math.round(random(5, height - height / 7));
      var cp1 = createSprite(randy4, 20, 20, 20);
      cp1.scale = 0.49;
      cp1.addImage(cpimg);
      cp1.velocityY = 13;
    ...
    if(ddr2.isTouching(flower1)){
      flowerscore = flowerscore +1 ;
      flowerGroup.destroyEach();
    }

        if(ddr2.isTouching(clock1)){
     clockscore =clockscore +1 ;
      clockGroup.destroyEach();
    }

        if(ddr2.isTouching(cp1)){
      cpscore = cpscore +1 ;
      cpGroup.destroyEach();
    }

    if (flowerscore === 1 && cpscore === 1 && clockscore === 1 || keyWentDown("down")) {
